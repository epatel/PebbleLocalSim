...
"""
	...
	GLOBALS_OUTLINE = """#include <stdbool.h>
...
"""
	...
	def _generate_globals(self):
		out_path = os.path.join(self._out_path, 'build', 'tempLocal', 'src', 'globals.auto.c')
		try:
			out_handle = open(out_path, 'wb')
		except OSError as e:
			logging.error('Couldn\'t open  file "%s": [Errno %d] %s',
			              out_path, e.errno, e.strerror)
			return False

		res_strings = ""
		if not self._json.has_key('watchapp'):
			logging.error('appinfo.json invalid (No object "watchapp" was found)')
			return False

		if not self._json['watchapp'].has_key('watchface'):
			logging.error('appinfo.json invalid (No object "watchface" was found in "watchapp")')
			return False

		if self._json['watchapp']['watchface']:
			res_strings += "bool g_is_fullscreen = true;\n"
		else:
			res_strings += "bool g_is_fullscreen = false;\n"

		out_handle.write(ResourceCompiler.GLOBALS_OUTLINE % res_strings)
		out_handle.close()
